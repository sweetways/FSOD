...
def parse_args():
    ...
    parser.add_argument(
        "--datasets_dir", type=str, default=datasetsDir, help="Path to the datasets directory"
    )
...
def generate_seeds(args):
    ...
    for year in [2007, 2012]:
        data_file = os.path.join(args.datasets_dir, "VOC{}/ImageSets/Main/trainval.txt".format(year))
        with PathManager.open(data_file) as f:
            fileids = np.loadtxt(f, dtype=np.str).tolist()
        data.extend(fileids)
    for fileid in data:
        year = "2012" if "_" in fileid else "2007"
        dirname = os.path.join(args.datasets_dir, "VOC{}".format(year))
    ...
    for i in range(args.seeds[0], args.seeds[1]):
            ...
            for j, shot in enumerate(shots):
                ...
                for s in shots_c:
                    if s not in c_data:
                        ...
                        name = os.path.join(args.datasets_dir, "{}/JPEGImages/{}".format(year, file))
                        c_data.append(name)
                        for obj in tree.findall("object"):
        ...
        save_path = os.path.join(args.datasets_dir, "vocsplit/seed{}".format(i))
